...
def BT_wCost(tradingRec):
    ...
    for k, v in tRec['Type_Enter'].value_counts().items():
        ...
        eMat['pct|Win|Enter:{}'.format(k0)] = gfc.true_pct(tyen_idx & win_idx)
        eMat['pct|Loss|Enter:{}'.format(k0)] = gfc.true_pct(
            tyen_idx & loss_idx)
        eMat['PnL|Enter:{}'.format(k0)] = tRec.loc[tyen_idx, 'GrossPnL'].sum()
        eMat['AvgRet|Enter:{}'.format(
            k0)] = tRec.loc[tyen_idx, 'RReturn'].mean()

    for k, v in tRec['Type_Exit'].value_counts().items():
        eMat['pct|Exit:{}'.format(k)] = float(v) / tRec.shape[0]
        tyex_idx = tRec['Type_Exit'] == k
        eMat['pct|Win|Exit:{}'.format(k)] = gfc.true_pct(tyex_idx & win_idx)
        eMat['pct|Loss|Exit:{}'.format(k)] = gfc.true_pct(tyex_idx & loss_idx)
...
def fix_signals(signals):
    ...
    :return:
    """
    pos = np.zeros(signals.shape)
    for i, sig in enumerate(signals):
        if sum(pos) == 0:
            pos[i] = sig
        elif sum(pos[:i - 1]) < 0 and sig == 1:
            pos[i] = sig
        elif sum(pos[:i - 1]) > 0 and sig == -1:
            pos[i] = sig
