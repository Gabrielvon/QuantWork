...
class clean_rdata():
    ...
    def remove_by_time(self, exchange=False, underlying=False):
        ...
        self.clean_df = pd.concat(slice_dfs).sort_index()
    ...
    def fix_duplicated_index(self, s=None):
        ...
        if np.any(is_dup):
            ...
            # new_idx[is_dup] = pd.to_datetime(fixed_idx)
            p.index = new_idx
...
# def __time_magic(x):
#     return pd.to_timedelta(x.isoformat().split('T')[1])


# def select_slice_time(df, time_slices, s=False, e=False):
#     compare_idx = np.array(map(__time_magic, df.index))
#     for ts_s, ts_e in time_slices:
#         if s:
#             tf_s = compare_idx >= pd.to_timedelta(ts_s)
#         else:
#             tf_s = compare_idx > pd.to_timedelta(ts_s)

#         if e:
#             tf_e = compare_idx <= pd.to_timedelta(ts_e)
#         else:
#             tf_e = compare_idx < pd.to_timedelta(ts_e)
#         cleaned_df = df[tf_s & tf_e]
#         yield cleaned_df
...
def splist(seq, size=10, fillna=True):
    ...
    if not fillna:
        new_list[-1] = tuple(i for i in new_list[-1] if i)
...
def pandas_fill(arr, method='ffill', axis=0):
    """Summary
    Fill nan values in arr wrapped up by pandas.
    ...
    """
    df = pd.DataFrame(arr.copy())
    if method in ['backfill', 'bfill', 'pad', 'ffill', None]:
        df.fillna(method=method, axis=axis, inplace=True)
    else:
        df.fillna(value=method, axis=axis, inplace=True)
    out = df.values.reshape(arr.shape)
    return out
...
def mem_usage(pandas_obj, unit='MB'):
    ...
    if unit.lower() == 'mb':
        return usage_b / 1024.0 ** 2  # convert bytes to megabytes
    elif unit.lower() == 'kb':
        return usage_b / 1024.0  # convert bytes to kilobytes
    else:
        return float(usage_b)


def convert_code(codes, flag=0):
    """Convert code format

    Args:
        codes (TYPE): Description
        flag (int, optional): 0, from '000001.SZ' to 'SZ000001'; 1, from 'SZ000001' to '000001.SZ';
    ...
    codes = list(codes)
    if flag == 0:
        return [co[-2:] + co[:6] for co in codes]
    elif flag == 1:
        return [co[2:] + '.' + co[:2] for co in codes]
