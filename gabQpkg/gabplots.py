...
def scale_axis(axes, orig_tick_lbls, interval=2, type='x'):
    """Summary

    Scale ticks with numeric label on x or y axis.

    Args:
        axes (TYPE): Description
        orig_tick_lbls (TYPE): Description
        interval (int, optional): Description
    ...
    """
    ax_lists = list(axes)
    tick_locs = range(0, len(orig_tick_lbls), 4)
    tick_lbls = orig_tick_lbls[tick_locs]
    if type == 'x':
        for ax in ax_lists:
...
def scale_index(index, n, form='%Y-%m-%d %H:%M:%S', dtype='str'):
    ...
    Generate sparse ticks and ticklabels based on timestamp values.

    Args:
        index (TYPE): Description
        n (TYPE): Description
        form (str, optional): Description
        dtype (str, optional): Description
