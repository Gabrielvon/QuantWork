...
def calc_factor1(df):
    # 基于订单簿的筹码倾向：以价格为标尺
    _obp = np.ravel(df.filter(regex='[ab]p\d'))
    _obv = np.ravel(df.filter(regex='[ab]v\d'))
    ob_agg = pd.Series({p: np.sum(_obv[_obp == p]) for p in np.unique(_obp)})
    ...
    dt, co = df[['dateint', 'code']].iloc[-1].values
    return dt, co, order_ratio


def calc_factor2(df):
    # 基于订单簿的筹码倾向：以位置为标尺
    _obp = df.filter(regex='[ab]p\d')
    argsort = np.argsort(_obp.iloc[0, :])
    _obp = _obp.iloc[:, argsort]
    _obv = df.filter(regex='[ab]v\d')
...
if __name__ == '__main__':
    ...
    # load data
    # --------------------------------------------------------------------------------
    all_trading_dates = lut.get_trading_dates()[-126:]
    ...
    selected_codes_numstr = hs300a[0].tolist()

    # import datetime
    # start = rawdata['datetime'].iloc[0]
    # end = rawdata['datetime'].iloc[-1]
    # date_generated = [start + datetime.timedelta(seconds=x) for x in range(0, (end - start).seconds)]

    # all_trading_dates = all_trading_dates[-5:]
    selected_codes = selected_codes[-30:]

    result_list = Parallel(n_jobs=12, verbose=10)(delayed(lut.load_orderbook)(code=co, date=d) for d in all_trading_dates for co in selected_codes)
    ...
    deal_list = Parallel(n_jobs=12, verbose=10)(delayed(lut.load_deal_data)(code=co, date=d) for d in all_trading_dates_int for co in selected_codes_numstr)
    # deal_list = [lut.load_deal_data(code=co, date=d) for d in all_trading_dates_int for co in selected_codes_numstr]
    rawdeal = pd.concat(deal_list)
    rawdeal['code'] = rawdeal['code'].map(code_type_map)
    gooddeal = rawdeal[(rawdeal['timeint'] >= 93000) & (rawdeal['timeint'] <= 145700)].copy()

    day_close = gooddata.groupby('code').apply(lambda x: x.groupby(pd.Grouper(key='datetime', freq='1d')).last())['new_price'].unstack('code').dropna(how='all')
    day_open = gooddata.groupby('code').apply(lambda x: x.groupby(pd.Grouper(key='datetime', freq='1d')).first())['new_price'].unstack('code').dropna(how='all')
    day_logret = np.log(day_close) - np.log(day_open)
    day_logret.index = day_logret.index.strftime('%Y%m%d').astype(int)
    day_logret_s1 = day_logret.shift(-1)

    factor1_list = Parallel(n_jobs=30, verbose=10)(delayed(calc_factor1)(g) for _, g in gooddata.groupby(['dateint', 'code']))
    factor1 = pd.DataFrame(factor1_list, columns=['datetime', 'code', 'factor1']).set_index(['datetime', 'code'])['factor1']
    factor2_list = Parallel(n_jobs=30, verbose=10)(delayed(calc_factor2)(g) for _, g in gooddata.groupby(['dateint', 'code']))
    factor2 = pd.DataFrame(factor2_list, columns=['datetime', 'code', 'factor2']).set_index(['datetime', 'code'])['factor2']

    input_data = day_logret.stack().to_frame('hmo')
    input_data['hmo_s1'] = day_logret_s1.stack()
    input_data['factor1'] = factor1
    input_data['factor2'] = factor2
    input_data = input_data.dropna()

    ic_comb = scs.spearmanr(input_data['hmo_s1'].values, input_data['factor1'].values)
    ic_xs = pd.Series({dt: scs.spearmanr(g['hmo_s1'].values, g['factor1'].values)[0] for dt, g in input_data.groupby('datetime')})    # cross-sectional ic overtime
    ic_xs.describe()
    ic_ts = pd.Series({co: scs.spearmanr(g['hmo_s1'].values, g['factor1'].values)[0] for co, g in input_data.groupby('code')})    # time-series ic by code
    ic_ts.describe()

    ic_comb = scs.spearmanr(input_data['hmo_s1'].values, input_data['factor2'].values)
    ic_xs = pd.Series({dt: scs.spearmanr(g['hmo_s1'].values, g['factor2'].values)[0] for dt, g in input_data.groupby('datetime')})    # cross-sectional ic overtime
    ic_xs.describe()
    ic_ts = pd.Series({co: scs.spearmanr(g['hmo_s1'].values, g['factor2'].values)[0] for co, g in input_data.groupby('code')})    # time-series ic by code
    ic_ts.describe()

    # dealbook: 成交主力
    for (dt, co), g in gooddeal.groupby(['dateint', 'code']):
        break

    cod
    g = lut.load_deal_data('000001', 20200106, verbose=1)
    df = g.copy()
    ...
    # factor_piv = input_data.pivot('datetime', 'code', 'factor1').ffill()

    factor_piv = input_data['factor1'].unstack('code')

    signal_piv_arr = np.stack([lut._generate_signal(ss, signal_ref=5, q_lower=0.4, q_upper=0.6, flag=1) for _, ss in factor_piv.iteritems()], axis=1)
    ...
    np.cumsum(np.mean(signal_piv_arr * logret_piv_s_good, axis=1))
