...
def black_scholes_call_value(S, K, R, T, V, Q):
    assert check_equal_length(S, K, R, T, V, Q), "Key values have different lengths."
    callResult = []
    for args in zip(S, K, R, T, V, Q):
        callResult.append(_black_scholes_call_value(*args))
    return np.array(callResult)
...
def black_scholes_put_value(S, K, R, T, V, Q):
    ...
    return np.array(putResult)


@jit(nopython=True)
def get_option_value(cp_flag, S, K, R, T, V, Q):
    assert check_equal_length(cp_flag, S, K, R, T, V, Q), "Key values have different lengths."
    option_values = []
...
def implied_volatility_series(cp_flag, S, K, r, t, q, option_price, upper=5, lower=0.000001, tol=1e-10, maxiter=np.inf):
    ...
    """
    assert check_equal_length(cp_flag, S, K, r, t, q, option_price), "Key values have different lengths."
    ivs = []
    for cp, S0, X0, r0, t0, q0, p0 in zip(cp_flag, S, K, r, t, q, option_price):
        if anynan([cp, S0, X0, r0, t0, q0, p0]) or (t0 == 0):
            iv = np.nan
        elif cp == 1:
...
def call_delta(S, K, R, T, V):
    ...
    """
    assert check_equal_length(S, K, R, T, V), "Key values have different lengths."
    call_delta_results = []
    for args in zip(S, K, R, T, V):
        call_delta_results.append(_call_delta(*args))
    return np.array(call_delta_results)
...
def put_delta(S, K, R, T, V):
    ...
    return np.array(put_delta_results)
...
def call_theta(S, K, R, T, V):
    ...
    return np.array(call_theta_results)
...
def put_theta(S, K, R, T, V):
    ...
    return np.array(put_theta_results)
...
def call_rho(S, K, R, T, V):
    ...
    return np.array(call_rho_results)
...
def put_rho(S, K, R, T, V):
    ...
    return np.array(put_rho_results)
...
def delta(cp_flag, S, K, R, T, V):
    ...
    """
    assert check_equal_length(cp_flag, S, K, R, T, V), "Key values have different lengths."
    deltas = []
    for cp, S0, X0, r0, t0, v0 in zip(cp_flag, S, K, R, T, V):
        if anynan([cp, S0, X0, r0, t0, v0]) or (t0 == 0):
            delta = np.nan
        elif cp == 1:
